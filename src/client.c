...
static bool skip_past_known_token(const char **p, const char *token)
{
...
static char *scan_for_search_path(struct MBuf *buf, const char type)
{
	...
	if (!skip_past_known_token(&query_str_p, "SET")) {
		log_debug("scan_for_search_path: SET not where it was expected");
		return NULL;
	}

	if (!skip_past_known_token(&query_str_p, "SEARCH_PATH")) {
		log_debug("scan_for_search_path: SEARCH_PATH not where it was expected");
		return NULL;
	}
